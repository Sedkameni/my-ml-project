...
This script performs comprehensive sentiment analysis on text data including:
- Text preprocessing (tokenization, stopword removal, lemmatization)
- Model training and evaluation (Naive Bayes, SVM)
...
ax3.set_title('Confusion Matrix - Naive Bayes', fontsize=14, fontweight='bold')
...
sns.heatmap(cm_svm, annot=True, fmt='d', cmap='Purples', ax=ax4)
ax4.set_title('Confusion Matrix - SVM', fontsize=14, fontweight='bold')
...
width = 0.35
ax5.bar(x - width / 2, nb_scores, width, label='Naive Bayes', color='#3498db')
ax5.bar(x + width / 2, svm_scores, width, label='SVM', color='#9b59b6')
ax5.set_ylabel('Score', fontsize=12)
ax5.set_title('Detailed Metrics Comparison', fontsize=14, fontweight='bold')
ax5.set_xticks(x)
ax5.set_xticklabels(metrics)
ax5.legend()
ax5.set_ylim([0, 1])

# 6. Text Length Distribution by Sentiment
ax6 = plt.subplot(2, 3, 6)
df['text_length'] = df['review'].str.len()
for sentiment in df['sentiment'].unique():
    data = df[df['sentiment'] == sentiment]['text_length']
    ax6.hist(data, alpha=0.5, label=sentiment, bins=30)
ax6.set_xlabel('Text Length (characters)', fontsize=12)
ax6.set_ylabel('Frequency', fontsize=12)
ax6.set_title('Text Length Distribution by Sentiment', fontsize=14, fontweight='bold')
ax6.legend()
...
for review in test_reviews:
    ...
    nb_prediction = nb_model.predict(vectorized)[0]
    svm_prediction = svm_model.predict(vectorized)[0]

    print(f"\nReview: '{review}'")
    print(f"  Naive Bayes: {nb_prediction}")
